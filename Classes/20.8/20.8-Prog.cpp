...
public:
	...
	void Show();//функция для просмотра атрибутов класса с полощью указателя
...
//селекторы
void Print::set_name(string a)
{
	name = a;
}
void Print::set_author(string b)
{
	author = b;
}
...
void List::Add()
{
	...
	cout << "1.Print" << endl;
	cout << "2.Magazin" << endl << "Enter com.num(y): " << endl;
	int y;
	cin >> y;
...
void List::Show()
{
	if (cur == 0)
	{
		...
		for (int i = 0; i < cur; i++)
		{
			(*p)->ShowObj();//вызов метода Show() (позднее связывание)
...
int main()
{
	...
	l.Add(p);//добавляем объект в вектор 
	p = b;//ставим указатель на объект класса Magazin
	l.Add(p);//добавляем объект в List
	l.Show();//вывод List
